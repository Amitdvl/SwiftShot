...
The current branch includes deterministic coverage for exact uneven offsets, sparse/repeated layouts, narrow unique markers, periodic dynamic rows, duplicates, noise, sticky edges, blank/ambiguous content, reverse motion, dimensions/scales, limits, cancellation/stale callbacks, source failure, HUD behavior, private lineage, and editor handoff. Native validation on the final signed Release app captured the 720 × 480 CSS-pixel browser fixture at 2×, kept the browser scroller focused, advanced from 1 to 4 accepted frame sections after a natural keyboard scroll, and produced a continuous **1440 × 1039** editor image. The HUD was absent from the captured pixels. An extended run reached 10 accepted sections and a continuous 1440 × 2039 result. Native Cancel produced no editor document.
