...
Scrolling Capture now keeps the selected viewport bright inside a passive full-display spotlight while dimming the surrounding screen. The selection edge is intentionally clear—there is no yellow rectangle—while the attached cumulative extent badge and short accepted-frame trail keep progress legible without obscuring the page. Finish and Cancel remain in a compact capsule outside the selection. The spotlight is nonactivating and click-through, so scrolling and pointer input continue to reach the source application; the control capsule stays one window level above it.

The stitch engine publishes measurement-only `ScrollingCaptureExtent` updates after verified accepts instead of rendering preview pixels during capture. Candidate seams are now proven against exhaustive row signatures, sampled RGB pixels, and high-information text/edge pixels. This prevents blank space and repeated card geometry from diluting the mismatches that distinguish a real overlap from a structural lookalike. Native validation on the installed signed app showed a sparse **484 CSS px** no-overlap jump remain at **1.0 screen · 1,171 px** with the explicit “Scroll a little slower” recovery state instead of appending duplicated content. The same run visually confirmed the borderless spotlight. Retained evidence is `build/scrolling-bug-final-native-start-3.png` and `build/scrolling-bug-final-native-rejected-jump.png`.

The focused Scrolling Capture slice passed **41/41 tests**, including exact accepted offsets, periodic dynamic rows, sticky regions, bucket-average pixel collisions, and sparse non-overlapping structural lookalikes. The full CI gate passed every phase and executed **401 tests with zero failures**; evidence is `build/ci/run.Vsx6X2`. The installed executable passed strict deep signature verification and has SHA-256 `5232e31cca001b7f22a03fb416af2270114eff0c060cc53612cfb1c13a236076`.

Local package: `build/release-artifacts/SwiftShot-scrolling-seam-fix-2026-09-13.zip`. ZIP integrity passed; SHA-256 is `cf2e1e380b7592fdaa2e95a8d1924c833febfeec7c2de25ecb3cf6132cc12eac`. This is a development-signed local artifact, not a notarized public release.
